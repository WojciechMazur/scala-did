...
object Indentity1Mediators {
  ...
  val indentity = DIDPeer2.makeIndentity(
    Seq(keyAgreement, keyAuthentication),
    Seq(DIDPeerServiceEncoded(Indentity0Mediators.indentity.id))
  )
...
object Indentity2Mediators {
  ...
  val indentity = DIDPeer2.makeIndentity(
    Seq(keyAgreement, keyAuthentication),
    Seq(DIDPeerServiceEncoded(Indentity1Mediators.indentity.id))
  )
  def indentityLayer: ULayer[Indentity] = ZLayer.succeed(indentity)
  def keyAgreement =
    OKPPrivateKey(
